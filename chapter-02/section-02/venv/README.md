...
When the Dockerfile sets `PATH="/opt/venv/bin:$PATH"`, commands resolve
from the virtual environment first:
...
Imports are resolved through Python's `sys.path`, which includes the
...
`venv` is bundled with Python, creates lightweight folder-based
environments, and is familiar to almost every Python developer.

### Cons

`venv` does not manage Python versions, does not install non-Python
system libraries, and does not provide a lockfile by itself.
