...
`[project.scripts]` registers the `historic_calculator` console entry
...
* Install the package and its pinned runtime dependencies. This
  registers the `historic_calculator` console script on your `PATH`.
...
```sh
historic_calculator COMMAND VECTOR
```
...
```sh
historic_calculator --help
historic_calculator max --help
```
...
```sh
historic_calculator mean 1.5,2.5,3.5
```
...
```sh
historic_calculator sum 10,20,30,40
```
