...
`[project.scripts]` registers the `historic-calculator` console entry
...
* Install the package and its pinned runtime dependencies. This
  registers the `historic-calculator` console script on your `PATH`.
...
```sh
historic-calculator COMMAND VECTOR
```
...
```sh
historic-calculator --help
historic-calculator max --help
```
...
```sh
historic-calculator mean 1.5,2.5,3.5
```
...
```sh
historic-calculator sum 10,20,30,40
```
...
```python
from my_package import run_calculator, make_vector
